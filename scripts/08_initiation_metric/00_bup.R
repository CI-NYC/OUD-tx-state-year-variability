...
rxl_buprenorphine <- 
  ...
  fmutate(moud_end_dt = RX_FILL_DT + days(DAYS_SUPPLY + 7)) |> 
...
otl_hcpcs_bup <-
  ...
  fmutate(moud_end_dt = fcase(
    form == "implant", moud_start_dt + 7 + 182,
    form == "injection", moud_start_dt + 7 + 30,
    form == "tablet", moud_start_dt + 7 
  )) |> 
  fselect(-form) |> 
  funique()
