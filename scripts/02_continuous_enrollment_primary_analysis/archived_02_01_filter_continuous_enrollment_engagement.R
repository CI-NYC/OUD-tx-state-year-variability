...
# Notes: This is checking whether patients have a MOUD initiation by 14 days after 
#         diagnosis, and remain continuously enrolled until MOUD initiation.
#         Also note, in this cohort, the latest allowed index date is Nov 17th 
# -------------------------------------
...
washout <- load_data("cohort_oud_hillary.fst", drv_root) |> as.data.table() |>
  filter(exclusion_nov17_washout == 0) |>
  select(BENE_ID, washout_start_dt, index_dt)

# filter to MOUD claims that come after OUD diagnosis
washout <- washout |>
  left_join(moud) |>
  filter(moud_start_dt >= index_dt & moud_start_dt <= index_dt + days(14))
...
washout <- washout[, .(
  ...
  exposure_end_dt = min(moud_start_dt) + days(30)
), by = BENE_ID]
...
for (i in seq_along(chunks)) {
  file_name <- paste0(drv_root,
                      "/tmp_engagement/enrollment_period_chunk_", 
                      i, ".rds"
  )
